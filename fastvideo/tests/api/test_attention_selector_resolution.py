...
The selector's contract after the refactor:
  ...
  * an active ``attention_backend_scope`` suppresses the env var unless
    ``consult_env=True`` — ``attention_backend_scope(None)`` therefore
...
def test_precedence_matrix_matches_previous_semantics(monkeypatch, forced, scoped, env, default) -> None:
    ...
    if scoped == "unset":
        ...
        with selector.attention_backend_scope(scoped, component="dit"):
            got = selector.get_attn_backend(default_backend=default, **KWARGS)
...
def test_scope_none_ignores_env_request(monkeypatch) -> None:
    ...
    with selector.attention_backend_scope(None, component="transformer"):
...
def test_interleaved_component_scopes_need_no_cache_clear(monkeypatch) -> None:
    ...
    for _ in range(2):
        with selector.attention_backend_scope(SDPA, component="student"):
            assert selector.get_attn_backend(**KWARGS) == "TORCH_SDPA"
        with selector.attention_backend_scope(None, component="teacher"):
...
def test_scope_is_exception_safe(monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="boom"):
        with selector.attention_backend_scope(SDPA, component="dit"):
            raise RuntimeError("boom")
    assert selector.active_attention_backend_scope() is None
    assert selector.get_attn_backend(**KWARGS) == "SAGE_ATTN"


def test_global_force_wins_without_cache_clear() -> None:
    with selector.attention_backend_scope(SAGE, component="dit"):
        assert selector.get_attn_backend(**KWARGS) == "SAGE_ATTN"
...
def test_scope_typo_fails_fast() -> None:
    with pytest.raises(ValueError, match="Unknown attention backend"):
        with selector.attention_backend_scope("flash_atn", component="dit"):
            pass


def test_consult_env_scope_keeps_env_visible(monkeypatch) -> None:
    monkeypatch.setenv("FASTVIDEO_ATTENTION_BACKEND", "SAGE_ATTN")
    with selector.attention_backend_scope(None, component="dit", consult_env=True):
        assert selector.get_attn_backend(**KWARGS) == "SAGE_ATTN"
